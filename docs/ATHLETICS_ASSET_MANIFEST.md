...
| --- | --- | ---: | ---: | ---: | ---: | --- |
| `creative-trio-ferris-wheel.glb` | Hero Ferris wheel silhouette above the Ferris & Coaster chapter | 841,568 B | 862,444 B | 1.005 × 0.208 × 1.135 | 10,672 | None; the source contained an empty image slot, so the mesh-only export avoids a broken texture request |
...
| `kenney-support-large.glb` | Two non-collidable coaster support silhouettes | 5,692 B | 12,864 B | 0.479 × 0.479 × 1.0 | 44 | 512 × 512 colormap |
...
the Athletics scene is created. Low-detail/performance settings skip selected
secondary coaster pieces, and every load is optional: if a GLB fails, the
named procedural fallback remains visible and the race continues.
...
client. Decorative stalls, Ferris geometry, coaster pieces, and supports do
not add physics bodies.
