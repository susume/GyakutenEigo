...
const formatDate = (value: string, timeZone = "Asia/Tokyo") => {
  ...
  return Number.isFinite(date.getTime()) ? new Intl.DateTimeFormat(undefined, { dateStyle: "full", timeStyle: "short", timeZone }).format(date) : "Release time unavailable";
};
const formatRemaining = (releaseAt: string, now: number) => {
  const releaseTime = new Date(releaseAt).getTime();
  if (!Number.isFinite(releaseTime)) return "Release time unavailable";
...
export default function TournamentStudyPage({ tournamentId }: { tournamentId: string }) {
  ...
  useEffect(() => {
    void api(`/api/tournament-study/${encodeURIComponent(tournamentId)}`)
      ...
      .catch((err: unknown) => setError(err instanceof ApiError ? err.message : "This study pack is not available."));
  }, [tournamentId]);
  useEffect(() => {
  ...
  if (error) return <section className="tournament-study-public"><div className="tournament-study-public-card"><LockKeyhole size={30} /><span className="eyebrow">QuizStrike Tournament Center</span><h1>Study pack unavailable</h1><p>{error}</p></div></section>;
