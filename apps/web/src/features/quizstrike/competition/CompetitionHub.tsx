...
const statusLabel: Record<CompetitionStatus, string> = {
  DRAFT: "Draft", ANNOUNCED: "Coming soon", REGISTRATION_OPEN: "Registration open", REGISTRATION_CLOSED: "Registration closed", STUDY_PERIOD: "Study period", CHECK_IN: "Check-in", LIVE: "Live now", COMPLETED: "Complete", CANCELLED: "Cancelled"
};
...
const requestError = (error: unknown) => error instanceof ApiError || error instanceof Error ? error.message : "We couldn’t load the competition details.";
  ...
  return <article className={`competition-card${featured ? " competition-card-featured" : ""}`}>
    ...
    <div className="competition-card-body">
      ...
      <div className="competition-card-grid">
        ...
        <Metric icon={<Clock3 size={16} />} label="Registration closes" value={competition.status === "REGISTRATION_OPEN" ? countdown(competition.registrationClosesAt, now) : formatDate(competition.registrationClosesAt, competition.timeZone)} />
        <Metric icon={<UsersRound size={16} />} label="Team places" value={`${competition.activeTeamSize} + ${competition.substituteLimit}`} />
      ...
      <div className="competition-card-footer"><span>{competition.division} · {competition.difficulty}</span><div className="button-row"><button className="text-button" onClick={onOpen}>View competition <ArrowRight size={15} aria-hidden="true" /></button>{competition.status === "REGISTRATION_OPEN" && <button className="primary small-button" onClick={onRegister}>Register a team</button>}</div></div>
    </div>
  </article>;
}
...
function RegistrationPanel({ competition, onClose, onTeacherLogin, onRegistered }: { competition: CompetitionCardData; onClose: () => void; onTeacherLogin: () => void; onRegistered: () => void }) {
  ...
  return <div className="competition-drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className="competition-drawer" role="dialog" aria-modal="true" aria-labelledby="register-team-title">
    <div className="competition-drawer-heading"><div><span className="eyebrow">Team registration</span><h2 id="register-team-title">Bring your team together.</h2><p>{competition.name}</p></div><button className="icon-button" aria-label="Close registration" onClick={onClose}><X size={20} /></button></div>
...
function StudyPanel({ competition, nowMs }: { competition: CompetitionCardData; nowMs: number }) {
  ...
  return <section className="competition-study-panel" aria-labelledby="study-panel-title"><div className="section-heading compact"><div><span className="eyebrow">Official study area</span><h2 id="study-panel-title">Learn the play before match day.</h2></div><BookOpenCheck size={26} aria-hidden="true" /></div>
    {!released ? <div className="study-lock-state"><div className="study-lock-icon"><LockKeyhole size={23} /></div><div><strong>Study pack opens in {countdown(competition.studyPackReleaseAt, nowMs)}</strong><p>Everyone gets access at the same time: {formatDate(competition.studyPackReleaseAt, competition.timeZone, true)} <span>({competition.timeZone})</span>.</p></div><span className="study-release-badge">Not released</span></div> : <div className="study-active-state"><div className="study-active-head"><div><span className="released-kicker"><CheckCircle2 size={15} /> Released · v{competition.studyPack?.version}</span><p>{competition.studyPack?.words.length ?? 0} approved words · the tournament question bank stays private.</p></div><button className="secondary-button">Practice the pack <ArrowRight size={16} /></button></div><div className="study-word-preview">{competition.studyPack?.words.slice(0, 3).map((word) => <div className="study-word" key={word.id}><strong>{word.targetWord}</strong><span>{word.partOfSpeech} · {word.approvedTranslation}</span><p>{word.simpleDefinition}</p></div>)}</div></div>}
  </section>;
}

function Bracket({ competition }: { competition: CompetitionCardData }) {
  const matches = competition.matches;
  return <section className="competition-bracket-panel" aria-labelledby="bracket-title"><div className="section-heading compact"><div><span className="eyebrow">Live structure</span><h2 id="bracket-title">Bracket and standings</h2></div><span className="section-heading-note">{competition.matchFormat}</span></div>{matches.length === 0 ? <div className="empty-bracket"><Swords size={22} /><p>The bracket will appear after approved teams are seeded.</p></div> : <div className="bracket-layout"><div className="bracket-column"><span className="bracket-round-label">{matches[0]?.roundLabel ?? "Round 1"}</span>{matches.slice(0, 4).map((match) => <div className="bracket-match" key={match.id}><div><span>{match.homeTeamName ?? "TBD"}</span><strong>{match.result?.homeScore ?? "–"}</strong></div><div><span>{match.awayTeamName ?? "TBD"}</span><strong>{match.result?.awayScore ?? "–"}</strong></div><small>{formatDate(match.scheduledAt, competition.timeZone, true)} · {match.map}</small></div>)}</div><div className="standings-card"><div className="standings-heading"><Trophy size={18} /><span>Standings</span><small>Public view</small></div>{competition.teams.length === 0 ? <p>Approved teams will appear here.</p> : <ol>{competition.teams.slice(0, 6).map((team) => <li key={team.id}><span className="standings-rank">{team.seed ?? "—"}</span><span><strong>{team.teamName}</strong><small>{team.affiliation}</small></span><span className="standings-record">{team.registrationStatus === "APPROVED" ? "Ready" : "Pending"}</span></li>)}</ol>}</div></div>}</section>;
}
