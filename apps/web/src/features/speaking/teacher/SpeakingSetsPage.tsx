  ...
  return <div className="speaking-page-shell speaking-teacher-shell">
    <main className="speaking-teacher-layout">
      <section className="speaking-teacher-content speaking-sets-page">
        <button type="button" className="speaking-text-button" onClick={() => navigate("/speak/teacher")}><ArrowLeft size={16} aria-hidden="true" />My Tests</button>
        <div className="speaking-teacher-heading">
          <div><span className="speaking-eyebrow">Speaking Practice · Organization</span><h1>My Sets</h1><p>Group your tests by grade, unit, term, or class.</p></div>
        ...
        {loading ? <div className="speaking-empty-card"><p>Loading My Sets…</p></div> : sets.length ? <div className="speaking-set-grid">
          {sets.map((set, index) => <button type="button" className="speaking-set-card" key={set.id} onClick={() => navigate(`/speak/teacher/set/${set.id}`)}>
            ...
            <span className="speaking-set-card-copy"><strong>{set.name}</strong><span>{set.activityCount} test{set.activityCount === 1 ? "" : "s"}</span><small>{set.lastUsedAt ? `Last used ${speakingTeacherDate(set.lastUsedAt)}` : "Not used yet"}</small></span>
            <ChevronRight size={19} aria-hidden="true" />
          </button>)}
        </div> : <div className="speaking-empty-card"><img className="speaking-empty-art" src="/assets/speaking/empty-sets.webp" alt="" width={132} height={132} /><h2>Make your first Set</h2><p>My Sets help you keep a term or class together without changing your tests.</p><button type="button" className="speaking-primary-button" onClick={() => setShowCreate(true)}>Create a Set</button></div>}
      </section>
    </main>
  </div>;
}
...
export function SpeakingSetDetailPage({ navigate, setId }: { navigate: Navigate; setId: string }) {
  ...
  const deleteSet = async () => {
    if (working || !set || !window.confirm(`Delete “${set.name}”?\n\nThe Set will be removed. Its ${set.activities.length} tests will remain in your library.`)) return;
    setWorking(true);
