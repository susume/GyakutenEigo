  ...
  return <div className="speaking-page-shell speaking-teacher-shell">
    <main className="speaking-teacher-layout">
      <section className="speaking-teacher-content speaking-sets-page">
        <button type="button" className="speaking-text-button" onClick={() => navigate("/speak/teacher")}><ArrowLeft size={16} aria-hidden="true" />Speaking Tasks</button>
        <div className="speaking-teacher-heading">
          <div><span className="speaking-eyebrow">Speaking Tasks · Organization</span><h1>My Sets</h1><p>Group your speaking tasks by grade, unit, term, or class.</p></div>
        ...
        {showCreate && <form className="speaking-set-create-form" onSubmit={create}>
          ...
          <label>Description <span className="speaking-muted-copy">optional</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="The speaking tasks for this term." maxLength={500} rows={2} /></label>
        ...
        </div> : <div className="speaking-empty-card"><img className="speaking-empty-art" src="/assets/speaking/empty-sets.webp" alt="" width={132} height={132} /><h2>Make your first Set</h2><p>My Sets help you keep a term or class together without changing your tasks.</p><button type="button" className="speaking-primary-button" onClick={() => setShowCreate(true)}>Create a Set</button></div>}
      </section>
    </main>
  </div>;
}
...
export function SpeakingSetDetailPage({ navigate, setId }: { navigate: Navigate; setId: string }) {
  ...
  if (!set) return <div className="speaking-empty-page"><h1>Set not found</h1><p>{error}</p><button type="button" className="speaking-primary-button" onClick={() => navigate("/speak/teacher/sets")}>Back to My Sets</button></div>;
  return <div className="speaking-page-shell speaking-teacher-shell">
    <main className="speaking-teacher-layout">
      <section className="speaking-teacher-content speaking-set-detail-page">
        <button type="button" className="speaking-text-button" onClick={() => navigate("/speak/teacher/sets")}><ArrowLeft size={16} aria-hidden="true" />My Sets</button>
        <div className="speaking-teacher-heading"><div><span className="speaking-eyebrow">My Set · {set.activities.length} task{set.activities.length === 1 ? "" : "s"}</span><h1>{set.name}</h1><p>{set.description || "Organize a sequence of classroom speaking tasks."}</p></div><div className="speaking-heading-actions"><button type="button" className="speaking-outline-button" onClick={() => setEditing((open) => !open)}><Pencil size={16} aria-hidden="true" />Edit Set</button><button type="button" className="speaking-outline-button" onClick={() => void download()}><Download size={16} aria-hidden="true" />Download CSV</button><details open={menuOpen} onToggle={(event) => setMenuOpen(event.currentTarget.open)}><summary aria-label="Set actions"><ChevronDown size={17} aria-hidden="true" /></summary><div className="speaking-overflow-menu"><button type="button" onClick={() => void deleteSet()}><Trash2 size={15} aria-hidden="true" />Delete Set</button></div></details></div></div>
        {error && <p className="speaking-error" role="alert">{error}</p>}
        {set.focus && <aside className="speaking-set-focus-card"><span className="speaking-card-kicker">Shared focus</span><p>{set.focus}</p><small>Optional focus for feedback and review across tasks in this Set.</small></aside>}
        {editing && <form className="speaking-set-edit-form" onSubmit={updateSet}><label>Set name<input value={name} onChange={(event) => setName(event.target.value)} maxLength={120} /></label><label>Description<textarea value={description} onChange={(event) => setDescription(event.target.value)} maxLength={500} rows={2} /></label><label>Shared focus <span className="speaking-muted-copy">optional</span><textarea value={focus} onChange={(event) => setFocus(event.target.value)} placeholder="Pay particular attention to follow-up questions and clarification." maxLength={500} rows={2} /><small className="speaking-form-help">Optional focus for feedback and review across tasks in this Set.</small></label><div className="speaking-form-actions"><button type="button" className="speaking-outline-button" onClick={() => setEditing(false)}>Cancel</button><button type="submit" className="speaking-primary-button" disabled={!name.trim() || working}>Save changes</button></div></form>}
        <section className="speaking-set-section"><div className="speaking-section-title"><div><span className="speaking-card-kicker">In this Set</span><h2>Speaking Tasks</h2></div><span>{set.activities.length} total</span></div>{set.activities.length ? <div className="speaking-set-member-list">{set.activities.map((item, index) => <article className="speaking-set-member" key={item.activity.id}><GripVertical size={17} aria-hidden="true" /><span className="speaking-set-position">{index + 1}</span><img src={item.activity.scenarioResources?.imageSrc ?? "/assets/speaking/scenario-introduction.webp"} alt="" loading="lazy" /><div className="speaking-set-member-copy"><strong>{item.activity.title}</strong><span><span className={`speaking-mode-badge speaking-mode-badge-compact speaking-mode-${item.activity.mode}`}>{speakingModeLabel(item.activity.mode)}</span> · {speakingScenarioResources(item.activity.scenarioResources).category ?? "Everyday Communication"} · {formatDuration(item.activity.durationSeconds)}</span><small>{item.sessionCount} session{item.sessionCount === 1 ? "" : "s"}{item.lastSessionAt ? ` · Last used ${speakingTeacherDate(item.lastSessionAt)}` : ""}</small></div><div className="speaking-set-member-actions"><button type="button" disabled={working} onClick={() => void launch(item.activity.id)} aria-label={`Launch ${item.activity.title} from Set`}>Launch</button><button type="button" onClick={() => move(index, -1)} disabled={index === 0 || working} aria-label={`Move ${item.activity.title} up`}>↑</button><button type="button" onClick={() => move(index, 1)} disabled={index === set.activities.length - 1 || working} aria-label={`Move ${item.activity.title} down`}>↓</button><button type="button" onClick={() => navigate(`/speak/teacher/activity/${item.activity.id}`)} aria-label={`Open ${item.activity.title}`}><ChevronRight size={17} aria-hidden="true" /></button><button type="button" onClick={() => void remove(item.activity.id)} disabled={working} aria-label={`Remove ${item.activity.title} from Set`}><Trash2 size={16} aria-hidden="true" /></button></div></article>)}</div> : <div className="speaking-empty-card"><h3>This Set is empty</h3><p>Add a speaking task below to start organizing the class.</p></div>}</section>
        <section className="speaking-set-section"><div className="speaking-section-title"><div><span className="speaking-card-kicker">Add from your library</span><h2>More Speaking Tasks</h2></div><label className="speaking-inline-search"><span className="sr-only">Search Speaking Tasks</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tasks" /></label></div>{available.length ? <div className="speaking-set-add-list">{available.map((activity) => <article key={activity.id}><img src={activity.scenarioResources?.imageSrc ?? "/assets/speaking/scenario-introduction.webp"} alt="" loading="lazy" /><div><strong>{activity.title}</strong><span>{activity.scenario}</span></div><button type="button" className="speaking-outline-button" onClick={() => void add(activity.id)} disabled={working}><Plus size={15} aria-hidden="true" />Add</button></article>)}</div> : <p className="speaking-muted-copy">Everything in your Set is already in Speaking Tasks, or nothing matches the search.</p>}</section>
      </section>
    </main>
  </div>;
}
