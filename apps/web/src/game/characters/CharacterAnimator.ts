...
export class CharacterAnimator {
  ...
  update(parts: CharacterAnimationParts, state: CharacterAnimationState) {
    ...
    parts.root.position.y = THREE.MathUtils.lerp(parts.root.position.y, state.crouching ? -0.3 : 0, 0.12);
    ...
    parts.leftShin.rotation.x = THREE.MathUtils.lerp(
      ...
      state.crouching ? 0.92 : Math.max(0, -swing) * 0.72,
      0.2
    );
    ...
    parts.leftArm.rotation.x = state.carryingObjective ? -0.72 : 0.46 - swing * 0.06;
    parts.rightArm.rotation.x = 0.58 + swing * 0.04;
    ...
    parts.torso.rotation.x = idle + Math.min(0.1, state.speed * 0.008) + (state.crouching ? 0.12 : 0);
