...
export const speakingApi = {
  ...
  createSet: (body: { name: string; description?: string; focus?: string }) => api("/api/speaking/sets", { method: "POST", body: JSON.stringify(body) }),
  updateSet: (id: string, body: { name?: string; description?: string; focus?: string }) => api(`/api/speaking/sets/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify(body) }),
  ...
  contextImageUrl: (id: string) => `${getApiUrl()}/api/speaking/context-images/${encodeURIComponent(id)}`,
  uploadContextImage: (file: Blob) => api<{ image: { id: string; url: string; mimeType: string; byteLength: number; width: number; height: number } }>("/api/speaking/context-images", { method: "POST", headers: { "Content-Type": file.type }, body: file }),
  deleteContextImage: (id: string) => api(`/api/speaking/context-images/${encodeURIComponent(id)}`, { method: "DELETE" }),
  ...
  launchSession: (activityId: string, setId?: string) => api(`/api/speaking/activities/${encodeURIComponent(activityId)}/sessions`, { method: "POST", ...(setId ? { body: JSON.stringify({ setId }) } : {}) }),
  join: (code: string, identifier?: string, requestId?: string, joinToken?: string) => api("/api/speaking/join", { method: "POST", ...(requestId ? { headers: { "X-Speaking-Join-Id": requestId } } : {}), body: JSON.stringify({ code, identifier, ...(requestId ? { requestId } : {}), ...(joinToken ? { joinToken } : {}) }) }, { attemptTimeoutMs: SPEAKING_JOIN_REQUEST_TIMEOUT_MS }),
  ...
  help: (sessionId: string, token: string, signal?: AbortSignal) => api(`/api/speaking/sessions/${encodeURIComponent(sessionId)}/help`, { method: "POST", headers: speakingHeaders(token), signal }, { attemptTimeoutMs: SPEAKING_HELP_REQUEST_TIMEOUT_MS }),
