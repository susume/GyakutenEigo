...
test("speakingApi sends the dedicated participant token header for every protected operation", async () => {
  ...
  try {
    ...
    await speakingApi.turn("session-1", "participant-token", { audio: new Blob(["audio"], { type: "audio/webm" }), requestId: "turn-2", speechDetected: false, audioDurationMs: 1_200 });
    ...
    assert.equal(new Headers(audioRequest.headers).get("X-Speaking-Audio-Duration-Ms"), "1200");
